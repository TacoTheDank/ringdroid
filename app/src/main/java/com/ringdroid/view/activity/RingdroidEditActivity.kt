...
import com.ringdroid.*
import com.ringdroid.data.player.SamplePlayer
import com.ringdroid.view.widget.MarkerListener
import com.ringdroid.view.widget.WaveformListener
import com.ringdroid.data.model.soundfile.SoundFile
import com.ringdroid.data.player.SongMetadataReader
import com.ringdroid.view.dialog.AfterSaveActionDialog
import com.ringdroid.view.dialog.FileSaveDialog
import com.ringdroid.view.widget.MarkerView
import com.ringdroid.view.widget.WaveformView
import java.io.File
 ...
 */
class RingdroidEditActivity : Activity(), MarkerListener, WaveformListener {
...
// they create.
    ...
    override fun onActivityResult(requestCode: Int,
        ...
        if (requestCode == REQUEST_CODE_CHOOSE_CONTACT) { // The user finished saving their ringtone and they're
...
//
     ...
     */
    private fun loadGui() { // Inflate our UI from its XML layout description.
        ...
        mLoadSoundFileThread = object : Thread() {
            override fun run() {
                try {
                    ...
                    val runnable = Runnable { showFinalAlert(e, resources.getText(R.string.read_error)) }
                    mHandler!!.post(runnable)
                    return
                }
                mProgressDialog!!.dismiss()
...
// On the other hand, if the text is big enough, this is good enough.
    ...
    private fun updateDisplay() {
        ...
        if (startX + mStartMarker!!.width >= 0) {
            if (!mStartVisible) { // Delay this to avoid flicker
                mHandler!!.postDelayed({
                    mStartVisible = true
                    mStartMarker!!.alpha = 1f
                }, 0)
        ...
        if (endX + mEndMarker!!.width >= 0) {
            if (!mEndVisible) { // Delay this to avoid flicker
                mHandler!!.postDelayed({
                    mEndVisible = true
                    mEndMarker!!.alpha = 1f
                }, 0)
    ...
    private val mTimerRunnable: Runnable = object : Runnable {
        override fun run() { // Updating an EditText is slow on Android.  Make sure
// we only do the update if the text has actually changed.
    ...
    private fun onPlay(startPosition: Int) {
        ...
        if (mPlayer == null) { // Not initialized yet
            return
        }
        try {
    ...
    private fun showFinalAlert(e: Exception?, message: CharSequence) {
        ...
        AlertDialog.Builder(this@RingdroidEditActivity)
                .setTitle(title)
                .setMessage(message)
                .setPositiveButton(
...
// directly to the sdcard
        ...
        for (i in 0..99) {
            ...
            try {
                ...
                f.close()
            } catch (e: Exception) { // Good, the file didn't exist
                path = testPath
                break
            }
        }
    ...
    private fun saveRingtone(title: CharSequence) {
        ...
        mSaveSoundFileThread = object : Thread() {
            override fun run() { // Try AAC first.
                ...
                } catch (e: Exception) { // log the error and try to create a .wav file instead
                    if (outFile.exists()) {
                        outFile.delete()
                    }
                ...
                if (fallbackToWAV) {
                    ...
                    try { // create the .wav file
                        mSoundFile!!.WriteWAVFile(outFile, startFrame, endFrame - startFrame)
                    } catch (e: Exception) { // Creating the .wav file also failed. Stop the progress dialog, show an
// error message and exit.
                        ...
                        val runnable = Runnable { showFinalAlert(e, errorMessage) }
                        mHandler!!.post(runnable)
                        return
                    }
                }
